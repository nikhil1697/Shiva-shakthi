...
export default function SpecializedExcellence() {
  ...
  return (
    <section id="wellness" className="relative overflow-hidden bg-turquoise-brand py-24 lg:py-32 text-white">
      {/* Background Grid Pattern Overlay */}
      <div className="absolute inset-0 grid-bg-overlay opacity-20 pointer-events-none z-0"></div>
 
      {/* Dynamic Ambient Background Glows */}
      <div className="absolute top-0 right-1/4 h-[400px] w-[400px] rounded-full bg-white/5 blur-[130px] pointer-events-none z-0"></div>
      <div className="absolute bottom-0 left-1/4 h-[400px] w-[400px] rounded-full bg-bronze-light/10 blur-[130px] pointer-events-none z-0"></div>
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="mb-20 text-center space-y-4">
          ...
          <p className="mx-auto max-w-xl text-xs font-bold uppercase tracking-widest text-bronze-light font-sans">
          ...
          {pathways.map((path) => {
            ...
            return (
              <motion.div
                ...
                whileHover={{ y: -8 }}
                className="group relative flex flex-col justify-between overflow-hidden rounded-3xl dark-glassmorphic-card p-8 cursor-pointer transition-colors duration-500 hover:bg-[#16827D]/45 border border-white/10"
              >
                ...
                <div className="absolute left-0 top-0 h-full w-1 origin-top scale-y-0 bg-gradient-to-b from-[#D8CEB8] to-white transition-transform duration-500 group-hover:scale-y-100 z-10"></div>
 
                <div className="relative z-10">
                  {/* Icon Emblem */}
                  <div className="mb-8 flex h-12 w-12 items-center justify-center rounded-2xl bg-white/5 text-bronze-light border border-white/15 transition-all duration-500 group-hover:bg-white group-hover:text-turquoise-brand group-hover:border-white group-hover:scale-105 shadow-inner">
                    <IconComp className="h-6 w-6 animate-pulse-slow" />
                  </div>
 
                  {/* Title & Tagline */}
                  <div className="space-y-3">
                    <h3 className="text-2xl font-serif font-semibold tracking-tight text-white transition-colors duration-300 group-hover:text-bronze-light">
                      {path.title}
                    </h3>
                  ...
                  <div className="mt-8 space-y-3.5 border-t border-white/10 pt-6">
                    {path.bullets.map((bullet, index) => (
                      <div key={index} className="flex items-center gap-3 text-xs font-bold text-gray-200 uppercase tracking-wider font-sans">
                        <span className="flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-white/5 border border-white/10 group-hover:border-bronze-light/50 transition-colors">
                          <Check className="h-3 w-3 text-bronze-light" />
                        </span>
                        <span>{bullet}</span>
                      </div>
                    ))}
                ...
                <div className="mt-8 pt-4 border-t border-white/5 flex items-center justify-between text-[10px] font-bold tracking-widest text-bronze-light uppercase font-sans overflow-hidden z-10">
                  ...
                  <ChevronRight className="h-4 w-4 transform transition-all duration-500 group-hover:translate-x-1 group-hover:text-bronze-light" />
                </div>
              </motion.div>
            );
          })}
